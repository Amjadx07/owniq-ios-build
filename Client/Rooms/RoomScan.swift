...
final class PublicRoomScanModel: ObservableObject {
    ...
    func attach(_ view: RoomCaptureView) { captureView = view }
    ...
    func didUpdate(_ room: CapturedRoom) {
        ...
        if coverage < 35 { instruction = "Continue : montre les murs et les coins." }
        else if coverage < 65 { instruction = "Scanne plus haut, plus bas et autour des meubles." }
        else if coverage < 85 { instruction = "Complète les zones manquantes et les objets importants." }
        else { instruction = "Bonne couverture. Tu peux terminer." }
    }

    func didReceive(_ value: RoomCaptureSession.Instruction) {
        switch value {
        case .moveCloseToWall: instruction = "Rapproche-toi d'un mur."
        case .moveAwayFromWall: instruction = "Recule légèrement."
        case .slowDown: instruction = "Ralentis."
        case .turnOnLight: instruction = "La pièce est sombre : augmente la lumière."
        case .lowTexture: instruction = "Zone difficile : vise davantage de détails."
        case .normal: break
...
struct RoomScanScreen: View {
    ...
    var body: some View {
        ZStack {
            ...
            if RoomCaptureSession.isSupported {
                PublicRoomCaptureRepresentable(model: model).ignoresSafeArea()
            } else {
                unsupported
            }
    ...
    private var hud: some View {
        VStack(spacing: 13) {
            ...
            Text(statusTitle).font(.headline).multilineTextAlignment(.center)
    ...
    private var unsupported: some View {
        VStack(spacing: 14) {
            ...
            Text("Scan 3D indisponible").font(.title2.bold())
    ...
    private func saveRoom() {
        ...
        let id = UUID()
        let usdzURL = store.newRoomURL(id: id)
        let jsonURL = store.newRoomJSONURL(id: id)
        ...
        do {
            ...
            store.protectFile(usdzURL)
            store.protectFile(jsonURL)
            ...
            let roomItems = room.objects.prefix(60).map { object in
                VaultItem(
                    ...
                    roomID: id,
                    source: .room3D,
                    notes: "Détecté dans \(record.name)"
                )
            }
...
enum RoomObjectNaming {
    static func name(_ category: CapturedRoom.Object.Category) -> String {
        ...
        case .oven: "Four à confirmer"
        case .refrigerator: "Réfrigérateur à confirmer"
        case .sink: "Évier"
        case .sofa: "Canapé"
        case .stairs: "Escalier"
        case .storage: "Rangement"
        case .stove: "Plaque ou cuisinière à confirmer"
        case .table: "Table"
        case .television: "Télévision"
        case .toilet: "Toilettes"
        case .washerDryer: "Lave-linge ou sèche-linge à confirmer"
        @unknown default: "Objet à confirmer"
        }
    }

    static func category(_ category: CapturedRoom.Object.Category) -> String {
        switch category {
        case .television: "Informatique"
        case .oven, .refrigerator, .sink, .stove, .dishwasher: "Cuisine"
        case .bed, .chair, .sofa, .storage, .table: "Mobilier"
        default: "Maison"
        }
    }
